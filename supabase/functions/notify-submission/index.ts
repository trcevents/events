...
const STORAGE_PUBLIC_BASE =
  "https://xdjbgcqaynnzykrglgnf.supabase.co/storage/v1/object/public/game-submissions";
...
function formatMarketplacePreorder(record) {
  ...
  const itemsList = items
    .map(
      (i) =>
        `${escapeHtml(i.name)}${i.variant ? " (" + escapeHtml(i.variant) + ")" : ""} x${escapeHtml(i.qty)}`,
    )
    .join("<br>");
  return {
...
function formatSponsorInquiry(record) {
  ...
  const fieldsList = fields
    .map(
      (f) =>
        `<p><strong>${escapeHtml(f.label)}:</strong> ${escapeHtml(f.value)}</p>`,
    )
    .join("");
  return {
...
function formatDonation(record) {
  const fundLabel =
    record.fund === "scholarship" ? "Youth Scholarship Fund" : "General Fund";
  return {
...
function formatGameSubmission(record) {
  const photoUrl = record.photo_path
    ? `${STORAGE_PUBLIC_BASE}/${record.photo_path}`
    : null;
  const videoUrl = record.video_path
    ? `${STORAGE_PUBLIC_BASE}/${record.video_path}`
    : null;
  return {
...
function formatCompRequest(record) {
  ...
  return {
    ...
    html: `
      <h2>New Comp/Sellable Ticket Request — ${escapeHtml(record.event_name || "Charly Black")}</h2>
  ...
  event_notify_signups: {
    notifications: [
      ...
      },
      {
        ...
        from: (record) =>
          record.brand === "trc"
            ? "TRC Events <hello@selassiefest.com>"
            : "SelassieFest <hello@selassiefest.com>",
      },
    ],
  },
  comp_requests: {
    notifications: [
      {
        to: (record) => {
          ...
          if (
            record.approver_listed &&
            COMP_APPROVER_EMAILS[record.approver_name]
          ) {
