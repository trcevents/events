...
Deno.serve(async (req) => {
  ...
  if (!email || !code) {
    return new Response(JSON.stringify({ valid: false, error: "Missing email or code" }), { status: 400 });
  }
  ...
  if (error || !row) {
    return new Response(JSON.stringify({ valid: false, error: "No code on file for this email -- request a new one" }), { status: 200 });
  }

  if (row.attempts >= MAX_ATTEMPTS) {
    return new Response(JSON.stringify({ valid: false, error: "Too many attempts -- request a new code" }), { status: 200 });
  }

  if (new Date(row.expires_at) < new Date()) {
    return new Response(JSON.stringify({ valid: false, error: "That code expired -- request a new one" }), { status: 200 });
  }
  ...
  if (codeHash !== row.code_hash) {
    ...
    return new Response(JSON.stringify({ valid: false, error: "That code doesn't match" }), { status: 200 });
  }
